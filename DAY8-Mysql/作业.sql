...
insert into emp(id, username, password, name, gender, phone, job, salary, image, entry_date, create_time, update_time)
values (null,'admin','123456','管理员',1,'12312341234',2,5000,'10086.jpg','2026-7-14',now(),now());
-- 2. 批量往emp表中插入两条测试数据， 全部的字段都需要插入值 。
insert into emp(id, username, password, name, gender, phone, job, salary, image, entry_date, create_time, update_time) values
(null,'admin121','123456','管理员',1,'12312341236',2,5000,'10086.jpg','2026-7-14',now(),now()),
(null,'admin123','1234567','管理员2',1,'12312341235',3,8000,'10087.jpg','2026-7-14',now(),now());
...
update emp set entry_date='2018-09-10',password='1234657' where id in (6,7,9,12);
...
select name as 姓名,entry_date 入职日期 from emp where gender=2 and job is not null ;
-- 4. 查询已有的员工关联了 哪几种 职位(不要重复)
select job as 职位
from emp where job is not null and job !='' group by job;
-- 5. 查询 姓名 为 '童威' 或 入职时间在 '2000-01-01' 到 '2010-01-01' 的员工
select name
from emp where name='童威' and entry_date >='2000-01-01'and entry_date <='2010-01-01';
...
select *
from emp where password !='123456' or job is null ;
-- 10. 查询入职日期 在 '2000-01-01' (包含) 到 '2015-01-01'(包含) 之间的员工信息
select *
from emp where entry_date >='2000-01-01' and entry_date <='2015-01-01';
-- 11. 查询 入职时间 在 '2000-01-01' (包含) 到 '2015-01-01'(包含) 之间 且 性别为女 的员工信息
select *
from emp where entry_date >='2000-01-01' and entry_date <='2015-01-01' and gender = 2;
...
select *
from emp where name like '__' and gender=1 and salary>5000;
-- 14. 查询姓名中包含 '小' 的员工信息  且  入职时间在 '2008-01-01' 之后入职的员工
select *
from emp where name like '%小%' and entry_date >='2008-01-01';
-- 15. 查询出 性别为 男 , 或 入职时间在 '2010-01-01' 之后入职的员工
select *
from emp where gender=1 and entry_date >='2010-01-01';
-- 16. 查询有职位的，且姓名为两个字 的员工信息
select *
from emp where job is not null and name like '__';
-- 17. 查询所有的性别为男(gender 为 1)的 讲师 (job 为 2) 的员工信息
select *
from emp where gender=1 and job=2;
-- 18. 查询薪资小于 8000 或 入职时间在 2010-10-09 之后入职的员工信息
select *
from emp where salary<8000 and entry_date>='2010-10-09';
-- 19. 根据性别分组 , 统计男性和女性员工的数量
select gender,count(*)
from emp group by gender;
-- 20. 先查询入职时间在 '2015-01-01' (包含) 以前的员工 , 并对结果根据职位分组 , 获取员工数量大于等于2的职位
select job,count(*)
from emp where entry_date <='2015-01-01' group by job having count(*)>=2;
