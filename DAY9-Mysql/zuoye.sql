...
select e.dept_id,e.name from tb_emp e,tb_dept d where e.dept_id=d.id;

-- 2. 查询员工的姓名 , 及所属的部门名称 (显式内连接实现)
select e.dept_id,e.name from tb_emp as e inner join tb_dept as d on e.dept_id=d.id;

-- 3. 查询员工的 姓名、性别、职位、部门名称
select name,gender,job,dept_id from tb_emp;

-- 4. 查询 薪资 高于 10000 的员工的姓名、性别、职位、部门名称
select name,gender,job,dept_id from tb_emp where salary>10000;
...
-- 7. 查询部门表 所有 部门的名称, 和对应的员工名称
select d.name,e.name from tb_emp e,tb_dept d where e.dept_id=d.id;

-- 8. 查询 "教研部" 的所有员工信息
select d.id from tb_dept d where name='教研部';
select e.name
from tb_emp e,tb_dept d
where e.dept_id in (select d.id from tb_dept d where name='教研部');
...
-- 10. 查询 "教研部" 和 "咨询部" 的所有员工信息
select d.id from tb_dept d where name in ('教研部','咨询部');
select *
from tb_emp e
where e.dept_id in (select d.id from tb_dept d where name in ('教研部','咨询部'));
-- 11. 查询与 "韦一笑" 的入职日期 及 职位都相同的员工信息
select entrydate,job from tb_emp where name='韦一笑';
select *
from tb_emp
where (entrydate,job)=(select entrydate,job from tb_emp where name='韦一笑') and name!='韦一笑';
-- 12. 查询入职日期是 "2006-01-01" 之后的员工信息 , 及其部门信息
select *
from tb_emp e,tb_dept d
...
group by d.id having count(*) is not null ;
...
-- 15. 查询 "教研部" 员工的平均工资
select avg(salary) from tb_emp e,tb_dept d where d.name='教研部';

-- 16. 查询工资比 "俞莲舟" 高的员工信息。
select e.salary from tb_emp e where name='俞莲舟';
select *
from tb_emp e where salary>(select e.salary from tb_emp e where name='俞莲舟');
-- 17. 查询 工资 比该企业员工的平均薪资 还要高的员工信息
select avg(salary) from tb_emp;
select e.*
from tb_emp e where salary>(select avg(salary) from tb_emp);
-- 18. 查询所有的部门信息, 并统计部门的员工人数
select d.id,d.name,count(*)
from tb_emp e,tb_dept d where e.dept_id=d.id group by d.id;

-- 19. 查询所有员工的 姓名, 工资 , 及 工资等级 (有难度的哦)
select e.name,e.salary,s.grade
from tb_emp e left join tb_salgrade s on salary between s.losal and s.hisal;
-- 20. 查询 "教研部" 所有员工的信息 及 工资等级 (有难度的哦)
select d.id from tb_dept d where d.name='教研部';
select *
from tb_emp e left join tb_salgrade s on salary
where e.dept_id=(select d.id from tb_dept d where d.name='教研部')
...
where e.dept_id is not null
  and salary > (
      select avg(salary)
      from tb_emp e2
      where e2.dept_id = e.dept_id -- 关联到外部员工所在的部门
  );
-- 22. 列出所有部门的详细信息(包括部门ID, 部门名称)和人数
select d.id,d.name,count(*)
from tb_emp e left join tb_dept d on e.dept_id=d.id
where e.dept_id is not null group by e.dept_id;

-- 23、取得每个薪资等级有多少员工  (有难度的哦)
select s.grade,count(*) emp_count
from tb_emp e left join tb_salgrade s on e.salary between s.losal and s.hisal
group by s.grade;

-- 24. 取得每个部门最高薪水的人员名称 (有难度的哦)
select dept_id,max(salary) from tb_emp where dept_id is not null group by dept_id;
select name
from tb_emp e,(select dept_id,max(salary) max from tb_emp where dept_id is not null group by dept_id) ms
